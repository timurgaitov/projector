...
## Gotchas
- Link testing (2026-07): ~11.7 Mbps sustained proven over 5 GHz Wi-Fi. Stress clip = `testsrc2` + `noise=alls=N` through `h264_videotoolbox` (N≈32→7M, 33→8M, 36-37→11-12M; bitrate is content-capped, `-b:v` barely steers it — hevc_videotoolbox behaves the same: 8M target → ~5.6M on noisy 1080p, ~3.3× realtime). Do NOT use x264 `nal-hrd=cbr` filler streams — the projector's decoder shows one frame and quits. videotoolbox overshoots `-maxrate` up to 2-3× on complex content.

...
- `plan()` ffprobe's the input and decides per stream: video is "fit" if 8-bit 4:2:0 h264 ≤1080p with projected bitrate ≤12.5Mbps (copy ceiling on trial — 8M sustained is proven; derived from size×8/duration when the container omits bit_rate; re-encoded audio's weight is swapped for the 256k AAC target), audio is fit if aac. Fit streams are copied, unfit ones re-encoded (`h264_videotoolbox` 8M; scaled into a 1920x1080 box). Streams are mapped by absolute index — skips attached_pic cover art; audio-less inputs work. Copy failures (bad timestamps etc.) retry as a full transcode; probe failure → full transcode. All encoded outputs are faststart MP4.
